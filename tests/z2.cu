...
__global__ void kernel_1(intfield5 link, double beta, int iter, float *cuda_action, int is_odd)
{
    ...
    for (dperp = 0; dperp < 4; dperp += 1) {
        if (dperp != d) {
            movedown(&id, dperp);
            int v1 = link.m[1*id.m[1*0] + link.size[0]*id.m[1*1] + link.size[0]*link.size[1]*id.m[1*2] + link.size[0]*link.size[1]*link.size[2]*id.m[1*3] + link.size[0]*link.size[1]*link.size[2]*link.size[3]*dperp];
            int v2 = link.m[1*id.m[1*0] + link.size[0]*id.m[1*1] + link.size[0]*link.size[1]*id.m[1*2] + link.size[0]*link.size[1]*link.size[2]*id.m[1*3] + link.size[0]*link.size[1]*link.size[2]*link.size[3]*d];
            staple = v1*v2;
            moveup(&id, d);
